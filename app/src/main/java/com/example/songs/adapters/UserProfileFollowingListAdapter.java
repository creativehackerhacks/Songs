...
public class UserProfileFollowingListAdapter extends RecyclerView.Adapter<UserProfileFollowingListAdapter.UserProfileFollowingListAdapterViewHolder> {

    private Context mContext;
    private List<UserProfileData> mUserProfileFollowingDataList = new ArrayList<>();

    private View.OnClickListener mItemClickListener;
    private View.OnClickListener mFollowIngBtnClickListener;

    public UserProfileFollowingListAdapter(Context mContext) {
        this.mContext = mContext;
    }

    public void loadAllUsersFollowingData(List<UserProfileData> userProfileFollowingData) {
        if(userProfileFollowingData == null) {
            return;
        }
        mUserProfileFollowingDataList.clear();
        mUserProfileFollowingDataList.addAll(userProfileFollowingData);
        notifyDataSetChanged();
    }

    public List<UserProfileData> getAllUsersFollowingData() {
        return mUserProfileFollowingDataList;
    }

    @NonNull
    @Override
    public UserProfileFollowingListAdapter.UserProfileFollowingListAdapterViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.row_profile_followling_list, parent, false);
        return new UserProfileFollowingListAdapterViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull UserProfileFollowingListAdapter.UserProfileFollowingListAdapterViewHolder holder, int position) {
        UserProfileData userProfileData = mUserProfileFollowingDataList.get(position);
        holder.bind(userProfileData);
    }

    @Override
    public int getItemCount() {
        return mUserProfileFollowingDataList.size();
    }
    ...
    public class UserProfileFollowingListAdapterViewHolder extends RecyclerView.ViewHolder {
        ...
        public UserProfileFollowingListAdapterViewHolder(@NonNull View itemView) {
            super(itemView);
